...
# make index database
out_filename=PROJDIR/ "data_train_smp_rag.csv"
...
def get_nearest_other_doc_id(query_text,edinetCode):
    # Max length of the KAM in dataset is 768, however multilingual-e5-large model can handle 512 tokens at once.
    # So we truncated the latter of the longer KAM because the former part of text is more important in KAM.
    ...
    similarities = []
    filename=PROJDIR / "data_train_smp_rag.csv"
...
data_train_pivot_smp=pd.read_csv(PROJDIR / "data_train_pivot_smp_0.csv")
...
data_train_pivot_smp[['id','nearest_id']].to_csv(PROJDIR  / "nearest_id.csv",index=False)
